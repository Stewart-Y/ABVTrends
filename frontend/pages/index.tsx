        ...
        <CardContent className="p-4 sm:p-6 relative z-10">
          <div className="flex items-start justify-between mb-3 sm:mb-4">
            ...
            <div className="text-right ml-3 sm:ml-4">
              <div className={cn(
                "text-3xl sm:text-4xl font-bold tabular-nums transition-colors",
          ...
          <div className="space-y-2">
            ...
            {Object.entries({
              ...
              <div key={key} className="flex items-center gap-2 sm:gap-3">
                <span className="text-xs text-muted-foreground w-10 sm:w-12">{key}</span>
                ...
                <span className="text-xs font-medium w-6 sm:w-8 text-right">{value.toFixed(0)}</span>
              </div>
            ))}
          </div>
        </CardContent>
...
export default function Dashboard() {
  ...
  return (
    <Layout
      title="Dashboard"
      subtitle={totalProducts > 0 ? 'Real-time alcohol trend analytics powered by AI' : 'Collecting data from media sources...'}
      headerActions={headerActions}
      testId="dashboard-page"
    >
      <div className="space-y-6 sm:space-y-8" data-testid="dashboard-header">
        {/* KPI Cards */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6" data-testid="kpi-cards-grid">
          {totalProducts > 0 ? (
            <>
              <StatCard
        ...
        {!isLoading && data && data.viral.length > 0 && (
          <div className="space-y-3 sm:space-y-4" data-testid="viral-section">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <h2 className="text-lg sm:text-xl lg:text-2xl font-bold flex items-center gap-2 sm:gap-3" data-testid="viral-section-title">
                  <svg className="w-6 h-6 sm:w-8 sm:h-8 text-red-400 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M13.5.67s.74 2.65.74 4.8c0 2.06-1.35 3.73-3.41 3.73-2.07 0-3.63-1.67-3.63-3.73l.03-.36C5.21 7.51 4 10.62 4 14c0 4.42 3.58 8 8 8s8-3.58 8-8C20 8.61 17.41 3.8 13.5.67zM11.71 19c-1.78 0-3.22-1.4-3.22-3.14 0-1.62 1.05-2.76 2.81-3.12 1.77-.36 3.6-1.21 4.62-2.58.39 1.29.59 2.65.59 4.04 0 2.65-2.15 4.8-4.8 4.8z"/>
                  </svg>
                  <span className="truncate">Viral Products</span>
                </h2>
                <p className="text-xs sm:text-sm text-muted-foreground mt-0.5 sm:mt-1">Score 90+ • Explosive growth</p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6" data-testid="viral-products-grid">
        ...
        {!isLoading && data && data.trending.length > 0 && (
          <div className="space-y-3 sm:space-y-4" data-testid="trending-section">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <h2 className="text-lg sm:text-xl lg:text-2xl font-bold flex items-center gap-2 sm:gap-3" data-testid="trending-section-title">
                  <svg className="w-6 h-6 sm:w-8 sm:h-8 text-orange-400 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M16 6l2.29 2.29-4.88 4.88-4-4L2 16.59 3.41 18l6-6 4 4 6.3-6.29L22 12V6z"/>
                  </svg>
                  <span className="truncate">Trending Products</span>
                </h2>
                <p className="text-xs sm:text-sm text-muted-foreground mt-0.5 sm:mt-1">Score 70-89 • Strong momentum</p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6" data-testid="trending-products-grid">
        ...
        {!isLoading && data && data.emerging.length > 0 && (
          <div className="space-y-3 sm:space-y-4" data-testid="emerging-section">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <h2 className="text-lg sm:text-xl lg:text-2xl font-bold flex items-center gap-2 sm:gap-3" data-testid="emerging-section-title">
                  <svg className="w-6 h-6 sm:w-8 sm:h-8 text-yellow-400 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
                  </svg>
                  <span className="truncate">Emerging Products</span>
                </h2>
                <p className="text-xs sm:text-sm text-muted-foreground mt-0.5 sm:mt-1">Score 50-69 • Watch closely</p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6" data-testid="emerging-products-grid">
        ...
        {!isLoading && totalProducts === 0 && signalsData?.data && (
          <div className="space-y-3 sm:space-y-4" data-testid="recent-activity-section">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <h2 className="text-lg sm:text-xl lg:text-2xl font-bold flex items-center gap-2 sm:gap-3" data-testid="recent-activity-title">
                  <svg className="w-6 h-6 sm:w-8 sm:h-8 text-blue-400 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
                  </svg>
                  <span className="truncate">Recent Activity</span>
                </h2>
                <p className="text-xs sm:text-sm text-muted-foreground mt-0.5 sm:mt-1">Latest mentions from media sources</p>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4" data-testid="recent-activity-grid">
                    ...
                    <div className="flex items-start gap-2 sm:gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <a href={signal.url} target="_blank" rel="noopener noreferrer" className="block">
                          <h3 className="font-medium text-xs sm:text-sm mb-1 line-clamp-2 group-hover:text-primary transition-colors">
                            {signal.title}
                          </h3>
                        </a>
